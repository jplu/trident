...
func TestRecord_IsGrandfathered(t *testing.T) {
	testCases := []struct {
		...
		{name: "Type is grandfathered", record: Record{Type: "grandfathered"}, expected: true},
		{name: "Type is redundant", record: Record{Type: "redundant"}, expected: true},
		{name: "Type is language", record: Record{Type: "language"}, expected: false},
		{name: "Type is extlang", record: Record{Type: "extlang"}, expected: false},
		{name: "Type is script", record: Record{Type: "script"}, expected: false},
		{name: "Type is region", record: Record{Type: "region"}, expected: false},
		{name: "Type is variant", record: Record{Type: "variant"}, expected: false},
		{name: "Type is empty", record: Record{Type: ""}, expected: false},
		{name: "Type is an arbitrary string", record: Record{Type: "other"}, expected: false},
	}
...
func Test_expandNumericRange(t *testing.T) {
	tests := []struct {
		...
		{name: "invalid end is not numeric", start: "11", end: "b3", wantErr: true},
		{name: "range too large", start: "0", end: "20001", wantErr: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func Test_expandAlphabeticRange(t *testing.T) {
	tests := []struct {
		...
		{name: "invalid start > end", start: "c", end: "a", wantErr: true},
		{name: "range too large", start: "aaaa", end: "zzzz", wantErr: true},
		{
...
func Test_expandRange(t *testing.T) {
	tests := []struct {
		...
		{name: "invalid format - empty string", rangeStr: "", wantErr: true},
		{name: "invalid format - unequal length", rangeStr: "a..bb", wantErr: true},
		{name: "invalid format - zero length parts", rangeStr: "..", wantErr: true},
		{name: "mixed type - alpha and numeric", rangeStr: "a..1", wantErr: true},
		{name: "error from numeric expander", rangeStr: "3..1", wantErr: true},
		{name: "error from alphabetic expander", rangeStr: "c..a", wantErr: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func Test_processAndAddRecord(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			record: Record{Type: "language", Subtag: "en"},
		...
		},
		{
			...
			record: Record{Type: "grandfathered", Tag: "i-klingon"},
			wantRegistry: &Registry{Records: map[string]Record{
		...
		},
		{
			...
			record: Record{Type: "region", Subtag: "001..003"},
			wantRegistry: &Registry{Records: map[string]Record{
		...
		},
		{
			...
			record: Record{Type: "private-use", Tag: "qaa..qac"},
			wantRegistry: &Registry{Records: map[string]Record{
		...
		},
		{
			...
			record:       Record{Type: "region", Subtag: "3..1"},
			wantRegistry: newTestRegistry(),
			wantErr:      true,
		},
		{
			name:         "invalid tag range",
			record:       Record{Type: "grandfathered", Tag: "z..x"},
			wantRegistry: newTestRegistry(),
			wantErr:      true,
		},
		{
...
`
	...
	tests := []parseRegistryTestCase{
		...
		{name: "empty registry", reader: strings.NewReader(""), wantRecordCount: 0},
		{
			name:         "registry with only file date",
			reader:       strings.NewReader("File-Date: 2024-01-01"),
			wantFileDate: "2024-01-01",
		},
		{name: "reader error", reader: errorReader{}, wantErr: true},
		{
